$(document).ready(function() {
    ...
    //Start: CRUD for vessel

    $(document).on('submit', 'form#vessel_add_form', function(e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success == true) {
                    $('div.vessel_modal').modal('hide');
                    toastr.success(result.msg);
                    if(typeof vessel_table !== 'undefined') {
                        vessel_table.ajax.reload();
                    }
                    var evt = new CustomEvent("vesselAdded", {detail: result.data});
                    window.dispatchEvent(evt);
                    //event can be listened as
                    //window.addEventListener("brandAdded", function(evt) {}
    ...
    //vessel table
    var vessel_table = $('#vessel_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '/vessel',
        columnDefs: [
    ...
    $(document).on('click', 'button.edit_vessel_button', function() {
        $('.edit_vessel_button .expected_date').datetimepicker({
            format: moment_date_format + ' ' + moment_time_format,
            ignoreReadonly: true,
        });
        $('div.vessel_modal').load($(this).data('href'), function() {
            $(this).modal('show');

            $('form#vessel_edit_form').submit(function(e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_vessel_button', function() {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_vessel,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then(willDelete => {
    ...
    $('#upload_document1').fileinput(fileinput_setting);
    $('#upload_document2').fileinput(fileinput_setting);
    $('#upload_document3').fileinput(fileinput_setting);
