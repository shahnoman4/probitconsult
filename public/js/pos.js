$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var arrival_charges = __read_number(tr.find('input.pos_vessel_arrival_charges'));

        var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
        var line_total = entered_qty * unit_price_inc_tax;
        var line_totals = line_total + arrival_charges;

        __write_number(tr.find('input.pos_line_total'), line_totals, false, 2);
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_totals, true));
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function() {
        ...
        __write_number(tr.find('input.pos_line_total'), line_totals);
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_totals, true));
        pos_each_row(tr);
        pos_total_row();
    ...
    $('table#pos_table tbody').on(
        'change',
        'input.pos_vessel_arrival_charges',
        function() {
            var tr = $(this).parents('tr');
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    var rentDayQtyGet = $('#rent_days_value').val();
                    console.log("line291=>",rentDayQtyGet);
                    $('.pos_quantity').val(rentDayQtyGet);
